<template>
  ...
  <div v-for="(pagina, key) in sections" :key="key">
    <a
      ...
      @mouseout="isHovered[key] = false"
      @click="created"
    >
      <div
        class="w-1 rounded-xl h-8 bg-transparent transition-colors duration-200 relative overflow-hidden"
        :class="{ 'bg-red-600': isHovered[key] }"
      >
        <div
          class="absolute top-0 left-0 w-full h-[102%] translate-y-full bg-red-600 transition-all duration-300"
        ></div>
      </div>
      <div
      ...
      >
        ...
        <svg class="h-5 w-5 fill-current text-red-500 transition-colors duration-200" viewBox="0 0 24 24">
